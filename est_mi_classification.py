...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoints', type=str, default='./checkpoints_cls/', help='location of model checkpoints')
    ...
    # expanded_shape = [args.batch_size,args.enc_in,args.seq_len,args.enc_in]
    ...
    if args.model == 'PatchTsT':
        latent_dim = args.d_model * (((args.seq_len - 16)//8)+1)
    elif args.model == "Crossformer":
        latent_dim = exp.model.head_nf
    else:
        latent_dim = args.d_model
    
    with torch.no_grad():
        for i, data_batch in enumerate(test_loader):
            batch_x,batch_y, padding_mask = exp.prepare_batch(data_batch)
            ...
            # preds.append(pred)
            # trues.append(true)
            seq_indices = torch.arange(args.seq_len,device=exp.device).view(1, 1, args.seq_len, 1).expand(batch_x.shape[0], args.enc_in, args.seq_len, args.enc_in)  # Shape: [N, F, L, F]
            ...
            # print('rp shape',batch_x.shape,rp3.shape,padding_mask.shape)
            padding_mask_expand = padding_mask.unsqueeze(1).expand(-1, args.enc_in, -1)       
            padding_mask_expand = padding_mask_expand.reshape(-1, padding_mask.shape[-1])
            ...
            new_outputs = torch.empty(3,batch_x.shape[0] * args.enc_in, latent_dim, args.enc_in,device=exp.device)
            with torch.no_grad():
                for n,rp in enumerate([rp1,rp2, rp3]):
                    ...
                    new_outputs[n] = exp.model.latent_rep(rp_inputs,padding_mask_expand).permute(0,2,1)
                    
                    # print('rp_outputs:',rp_outputs.shape)
            
            # new_outputs = new_outputs + [outputs]
            tot = torch.std(new_outputs,dim=0)
            tot = tot.reshape(batch_x.shape[0],args.enc_in,latent_dim,args.enc_in)
    ...
    with open('./eval_results/mi_cls_results.txt','a') as f:
        f.write(setting + "  \n")
